/*
LilyGo T3_V1.6.1, ESP32, TTGO with LoRa32 - 868-915 Mhz, Sensor Values simulation , Change packet values on specific Downlink packet value;
...
static const u1_t PROGMEM APPEUI[8] = {0xF2, 0x2E, 0xD5, 0xBB, 0xFC, 0xF9, 0x81, 0x60}; // TTN SERVER ONLY! -> Last Bytes: 0xD5, 0xB3, 0x70 | Instead: 0xF9, 0x81, 0x60 ;
void os_getArtEui(u1_t *buf) { memcpy_P(buf, APPEUI, 8); }

// This should also be in little endian format, see above.
static const u1_t PROGMEM DEVEUI[8] = {0x82, 0xE1, 0x26, 0x8B, 0x29, 0xF9, 0x81, 0x60};
void os_getDevEui(u1_t *buf) { memcpy_P(buf, DEVEUI, 8); }

// This key should be in big endian format (or, since it is not really a
// number but a block of memory, endianness does not really apply). In
// practice, a key taken from the TTN console can be copied as-is.
static const u1_t PROGMEM APPKEY[16] = {0xAD, 0x75, 0x5A, 0xC2, 0x0E, 0xD1, 0x78, 0x2D, 0x12, 0x5C, 0xC2, 0x81, 0x0F, 0x60, 0x9C, 0x93};
...
void lmicRxCallback(void *pUserData, uint8_t port, const uint8_t *pMsg, size_t nMsg)
{
  // display amount of received data
  if (nMsg)
    Serial.printf("Received %u byte(s) of payload on port %u\n", nMsg, port);
  else if (port)
    Serial.printf("Received empty message on port %u\n", port);
...
////NOT WORKING , FOR SOME REASON .... //////////////////////////////////
    // Read packet data
    //String packet1 = (char *)LMIC.frame + LMIC.dataBeg;
    //char buffer2[16] = "650 700 850 900";
    //char emptyString[16] = "";
    //Check current packet if it is equal to 250
    /*
    if (packet1 == "250")
    {
      strcpy(buffer, emptyString);
